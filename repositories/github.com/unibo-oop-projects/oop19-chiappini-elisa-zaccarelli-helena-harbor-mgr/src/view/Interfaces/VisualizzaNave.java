 ...
 * Frame di visualizzazione e cancellazione istanze di Nave
 */


public class VisualizzaNave extends JFrame {
private static final long serialVersionUID = 1L;
	...
	private void cancellaRecordSelezionato() {
        ...
        new Thread(() -> {
        	GestioneNave.creaIst().rimuoviNave(codice);
        	Collection<ImplNave> lista = GestioneNave.creaIst().getList();
	 ...
	 * @author Elisa Chiappini
	 */
	public void ricaricaTabella(Collection<ImplNave> lista) {
        ...
        String intestazione[] = new String[] {"Codice", "Nome Nave","Portata","Spazi","Bandiera","Tipo Merce Carico"};
        dtm.setColumnIdentifiers(intestazione);
        tabella.getTableHeader().setReorderingAllowed(false);
        tabella.setModel(dtm);
        for (ImplNave nave : lista) {
            dtm.addRow(new Object[] {nave.getCod(),nave.getNome(),nave.getPortata(),nave.getSpazi(),nave.getBandiera(),nave.getTipo() });
        }
	}
        
	
	public VisualizzaNave() {
    	this.setResizable(false);
    	this.setTitle("VISUALIZZAZIONE NAVI IN ARCHIVIO");
    	this.addWindowListener(new WindowAdapter() {
    		@Override
    		public void windowOpened(WindowEvent e) {
    			ricaricaTabella(GestioneNave.creaIst().getList());
    		}
    	});
        ...
        btnCerca.addActionListener ((a) -> {
            	ricaricaTabella(GestioneNave.creaIst().cerca(cerca.getText()));
        });
        btnCerca.setBounds(540, 10, 89, 23);
