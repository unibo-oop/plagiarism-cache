...
/**
 * The Class TQLevel2 is just a copy of the first level of the game Toa's Quest 2. The only difference from the first level is that the player is smaller.
 * This level was made to test how the game jumps to the next level.
 * 
 * @author Francesco Gori.
 */

public class TQLevel2 extends TQLevel {
	...
	public TQLevel2(Handler handler) {
		super(handler);
			...
			for(int y = 0; y<layer.getHeight(); y++) {
				for(int x = 0; x<layer.getWidth(); x++) {
					...
					if(red==255&&green==0&&blue==0) {
						Tahu player = new Tahu (x, y, 1, true, 0.5, 0.5, 1.0f, this.getHandler(), this.getHandler().getLayer(i));
