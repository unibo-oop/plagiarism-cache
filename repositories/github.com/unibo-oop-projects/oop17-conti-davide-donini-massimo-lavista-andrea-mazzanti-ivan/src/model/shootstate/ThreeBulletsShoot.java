...
/**
 * 
 * Implements behavior associated with a state: this behavior consists in firing three bullets spaced 20 degrees.
 *
 */
public final class ThreeBulletsShoot extends AbstractShoot {

    private static final int DEGREE_DISTANCE = 20;

    private ThreeBulletsShoot(final Optional<ShootState> nextState, final Optional<ShootState> previousState) {
        super();
    ...
    public List<Bullet> shoot(final Position spaceshipPosition, final double bulletDamage) {
        ...
        return Arrays.asList(new BulletImpl(new Circle(spaceshipPosition.getX(), spaceshipPosition.getY(), 
                RADIUS_BULLET_SHAPE), bulletSpeed, bulletDamage, BulletType.FRIENDLY), 
    ...
    public static class Builder {
        ...
        public ShootState build() {
            return new ThreeBulletsShoot(this.next, this.previous);
        }
    }
}
