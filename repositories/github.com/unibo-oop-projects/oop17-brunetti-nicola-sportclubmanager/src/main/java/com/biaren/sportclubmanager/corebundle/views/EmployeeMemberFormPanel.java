...
/**
 * Represent a form panel for a {@link EmployeeMember}
 * @author nbrunetti
 *
 */
public class EmployeeMemberFormPanel extends APersonFormPanel<EmployeeMember>{

    private VerticalLabelChoiceFieldBox<SportSocietyEmployee> role;
    
    /**
     * Creates a {@link EmployeeMemberFormPanel}, instantiate fields,
     * set the layout and apply validation to fields.
     * @param title of form
     * @param parentView form parent view
     */
    public EmployeeMemberFormPanel(final String title, final Pane parentView) {
        super(title, parentView);
        this.role = new VerticalLabelChoiceFieldBox<>(Fields.ROLE.getLabel(), Arrays.asList(SportSocietyEmployee.values()));
        this.setLayout();
        this.validateFields();
    }
    ...
    @Override
    public void fillFormWithData(final EmployeeMember entity) {
        super.fillFormWithData(entity);
        this.role.getField().setValue(entity.getEmployeeMemberRole());
    }
    ...
    protected void updateParentView() {
        ((EmployeeMemberPanel) this.parentView).updateView();
    }
    ...
    public enum Fields {
         ...
         */
        ROLE("employeeMemberRole", "Ruolo", Length.ROLE, s -> true);
