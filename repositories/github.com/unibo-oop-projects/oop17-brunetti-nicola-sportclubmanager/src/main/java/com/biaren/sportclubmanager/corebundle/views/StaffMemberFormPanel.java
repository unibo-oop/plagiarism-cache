...
/**
 * Represent a form panel for a {@link StaffMember}
 * @author nbrunetti
 *
 */
public class StaffMemberFormPanel extends APersonFormPanel<StaffMember> {
    
    private VerticalLabelChoiceFieldBox<SportSocietyStaffRole> role;
    
    /**
     * Creates a {@link StaffMemberFormPanel}, instantiate fields,
     * set the layout and apply validation to fields.
     * @param title of form 
     * @param parentView of this view
     */
    public StaffMemberFormPanel(final String title, final Pane parentView) {
        super(title, parentView);
        this.role = new VerticalLabelChoiceFieldBox<>(Fields.ROLE.getLabel(), Arrays.asList(SportSocietyStaffRole.values()));
        this.setLayout();
        this.validateFields();
    }
    ...
    @Override
    public void fillFormWithData(final StaffMember entity) {
        super.fillFormWithData(entity);
        this.role.getField().setValue(entity.getStaffMemberRole());
    }
    ...
    protected void updateParentView() {
        ((StaffMemberPanel) this.parentView).updateView();
    }
