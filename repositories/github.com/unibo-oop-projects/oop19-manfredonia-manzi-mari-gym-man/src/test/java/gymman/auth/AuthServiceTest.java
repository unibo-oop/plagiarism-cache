...
public class AuthServiceTest {
	...
	public void setUp() throws Exception {
		user1 = new User("pippo", "ROLE_1", "123", User.getHash("passpass"));
		User user2 = new User("pippo", "ROLE_2", "456", User.getHash("passpass"));
		...
		Mockito.when(userRepo.getUserById("123")).thenReturn(Optional.of(user1));
		Mockito.when(userRepo.hasUsername("pippo")).thenReturn(true);
		Mockito.when(userRepo.getUserByUsername("pippo")).thenReturn(Optional.of(user1));

		Mockito.when(userRepo.getUserById("456")).thenReturn(Optional.of(user2));
		Mockito.when(userRepo.hasUsername("pluto")).thenReturn(true);
		Mockito.when(userRepo.getUserByUsername("pluto")).thenReturn(Optional.of(user2));
