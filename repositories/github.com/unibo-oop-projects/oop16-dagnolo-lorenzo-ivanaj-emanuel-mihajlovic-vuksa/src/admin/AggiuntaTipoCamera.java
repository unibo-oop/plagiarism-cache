...
public class AggiuntaTipoCamera {
    ...
    public AggiuntaTipoCamera(Integer numeroCam) {
        this.numeroCamera = numeroCam;
        ...
        this.southPanel = new JPanel();
          ...
          HotelManager.create().removePriceDescriber(
          Hotel.instance().getPriceView(StayExtraPriceDescriber.class).stream()
          .filter(stay -> stay .getDescription().equals( /*stringa descrizione*/)).findAny().get()); per rimuovere supp this.str =
          Hotel.instance().getPriceView(StayExtraPriceDescriber.class).stream()
          .map(type ->
          type.getDescription()).collect(Collectors.toList()).toArray(new
          String[0]));
         
        
          this.labelType = new JLabel("Pensione attuale   " +
          Hotel.instance().getPriceView(StayTypePriceDescriber.class)
         .stream().filter(type ->
          type.getDescription().equals(this.numDoc)).findFirst().get());
          this.grid.gridx = 0; this.grid.gridy = 0; this.grid.insets = new
          Insets(10, 10, 10, 10); this.panel.add(this.labelType, this.grid);
        ...
        this.panel.add(this.label, this.grid);
         ...
         this.combo = new JComboBox<>(this.str);
          this.combo.setBackground(Color.YELLOW); this.grid.gridx = 1;
          this.grid.gridy = 1; this.grid.insets = new Insets(10, 10, 10, 10);
