...
/**
 * 
 * Class representing the player who has lost more matches.
 *
 */
public final class LostMatchesCountGlobalStatistic extends AbstractStatistic<String> {
    /**
     * the return value.
     */
     ...
     */
    public LostMatchesCountGlobalStatistic(final List<StatisticMatch> matches) {
        super("Player who has lost more games");
        if (matches == null) {
    ...
    private void initialize(final List<StatisticMatch> matches) {
        ...
        for (final String name : playerMatchCount.keySet()) {
            ...
            playerMatchCount.put(name, new MatchesCountStatistic(list).getValue() - new WonMatchesCountStatistic(list).getValue());
        }
        playerMatchCount.keySet().stream().filter(playerName -> !playerMatchCount.get(playerName).equals(0))
