...
/**
 * Questa interfaccia è usata per controllare una GUI, 
 * GUI che chiede ad un utente una sequenza di elementi del tipo X, 
 * con la possibilità di rifiutare un elemento dipendentemente da quale 
 * fosse il precedente (attraverso un filtro), 
 * di aggregare tutti gli elementi finora prodotti in un unico valore X, 
...
public interface ProgressiveAcceptor<X> {
  ...
  /**
   * This method accepts a new element of the sequence. 
   * An element is accepted if it is OK with respect to previous one 
   * according to the filter. Ideally elements are accepted in the order of position 0,1,2,3..
   * If one re-accepts a previous element, the subsequent part 
   * of the sequence is erased and needs to be re-accepted.
   * For instance, if the sequence accepts 10,20,30,40 in positions 0,1,2,3, 
   * and then one re-accepts 21 in position 1,
