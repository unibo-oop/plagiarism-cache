...
public class FirstBoss extends Entity {
	...
	public void render(Graphics g) {
		if (getVelX() < 0) { // se va verso sx
			g.drawImage(Game.firstBossSx.getBufferedImage(), x, y, width,
					height, null);
			if (getX() % (-3) == 0) {
				Game.handler.addEntity(new EnergyWave(0, getX() - 70, getY(),
						64, 64, true, Id.energyWaveBoss, handler));
			}
		} else {
			g.drawImage(Game.firstBossDx.getBufferedImage(), x, y, width,
					height, null);
			if (getX() % 3 == 0) {
				Game.handler.addEntity(new EnergyWave(1, getX() + 70, getY(),
						64, 64, true, Id.energyWaveBoss, handler));
			}
		}
	}
