...
/**
 * Tests {@link controller.TanksDealer}.
 */
public class TestTanksDealerSynchronization {
    ...
    /**
     * Tests synchronization between {@link controller.TanksDealer} and {@link controller.TanksAnalyst}.
     */
    @Test
    public void testTanksDealer() {
        Model model = null;
        try {
        ...
        for (int i = 0; i < model.getPlayers().size(); i++) {
            final TanksDealer tanksDealer = new TanksDealer(model, this.dummyView);
            tanksDealer.start();
            try {
                Thread.sleep(WAITING_TIME);
            } catch (InterruptedException e) { }
            if (model.getActualPlayer().playerType() == ControlType.HUMAN) {
                assertTrue("Error: TanksDealer not alive", tanksDealer.isAlive());
                assertSame("Error: TanksDealer not waiting", tanksDealer.getState(), Thread.State.WAITING);
            ...
            System.out.println("Thread state: " + tanksDealer.getState());
            assertSame("Error: TanksDealer not terminated", tanksDealer.getState(), Thread.State.TERMINATED);
            assertSame("Incorrect status", ControllerFactory.getController().getGameStatus(), Status.ATTACK);
            model.shiftPlayer();
        }
    }
