package ala.models;

import ala.models.LevelGeneratorPatternModel.OBJECTSTYPE;

/**
 * make DynamicGameObject move.
 * 
 */
public class HellShootingEnemyModel extends StandardEnemyModel {
    //Attributes:
    private static final OBJECTSTYPE TYPE = OBJECTSTYPE.SHOOTING_ENEMY;
    private static final double HEALTH = 10;
    private static final double DAMAGE_ON_CONTACT = 0;
    private static final double R = 0;
    private static final double DX = 0;
    private static final double DY = 0;
    private static final double DR = 0;
    private static final double WIDTH = 128;
    private static final double HEIGHT = 256;

    /**
     * range distance of alert.
     */
    public static final int SHOOTING_RANGE_ALERT = 50;

    //Constructors:
    /**
     * Constructor.
     * 
     * @param x
     * @param y
     * 
     */
    public HellShootingEnemyModel(final double x, final double y) {
        super(x, y, R, DX, DY, DR, TYPE, HEALTH, DAMAGE_ON_CONTACT, WIDTH, HEIGHT);
    }

    /**
     * Constructor.
     * 
     * @param x
     * @param y
     * @param r
     * @param dx
     * @param dy
     * @param dr
     * @param type
     * @param health
     * @param damageOnContact
     * 
     */
    public HellShootingEnemyModel(final double x, final double y, final double r, final double dx, final double dy, final double dr, final OBJECTSTYPE type, final double health, final double damageOnContact) {
        super(x, y, r, dx, dy, dr, type, health, damageOnContact, WIDTH, HEIGHT);
    }
}
