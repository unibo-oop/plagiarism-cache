...
public class King implements PieceType {
    ...
    public List<Box> possibleMoves(final Board b, final Box current, final Colour colour) {        
        ...
        if (this.pu.checkRange(x + 1, y)
                && this.pu.checkOpponent(x + 1, y, colour, b)) {
            moves.add(b.getBox(x + 1, y));
        }
        if (this.pu.checkRange(x, y + 1)
                && this.pu.checkOpponent(x, y + 1, colour, b)) {
        ...
        if (this.pu.checkRange(x + 1, y + 1)
                && this.pu.checkOpponent(x + 1, y + 1, colour, b)) {
